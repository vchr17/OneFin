...
class FavouritesAdapter(moneyList: MutableList<Money>, viewModel: FavouritesViewModel) :
    RecyclerView.Adapter<FavouritesAdapter.FavouritesHolder>() {
    private var list = moneyList
    private val vm = viewModel

    class FavouritesHolder(binding: RcValueBinding) :
    ...
    override fun onBindViewHolder(holder: FavouritesHolder, position: Int) {
        ...
        holder.itemView.setOnLongClickListener { view ->
            ...
            menu.inflate(R.menu.favourites_pop_up_menu)
            menu.setOnMenuItemClickListener { item ->
                when (item.itemId) {
                    R.id.delete_from_favourites -> {
                    ...
                    R.id.favorites_share_button -> {
                        vm.shareText(
                            parseText(
                                list[position].name, list[position].value, view.context
                            )
                        )
                        true
                    }
